...
def ndl = new XmlParser().parseText (writer.toString())
def rdf = new RDFDocument()



org.eclipse.ntis.model.ndl.Device.each{
	def dev = new Device()
...
org.eclipse.ntis.model.ndl.Location.each { 
	def l = new Location()
	it.attributes().each {key,value->
...
org.eclipse.ntis.model.ndl.Interface.each {
	def l = new Interface()
	it.attributes().each {key,value->
		handleAttributes(l, key, value)
	}
...
def handleAttributes(c, QName key,value){
